...
// 스위치 개수 18개 초과 : 
//  1) Node-RED Dashboard 개별스위치 Tab에 그룹 추가 - 그룹이름 : 번호목록 + 그룹 번호
//  2) 개별스위치 flow에 스위치 정보에 들어가는 Dashboad 위젯 추가 후 flow 설정 및 그룹 설정
//  3) switch.js 코드 수정 - 그룹 번호 설정에 추가된 그룹의 수만큼 else if문 추가, 동적 화면 적용에 추가된 그룹의 수만큼 show, hide 수정
//      ex) 스위치 개수 20개일 때
//          번호목록7 그룹 생성 
//          -> 위젯 추가 
//          -> 코드 수정 : show에 "개별스위치_번호목록 + (groupCount - 6)" 추가, 
//                         hide에 "개별스위치_번호목록 + (groupCount + 6)" 추가
